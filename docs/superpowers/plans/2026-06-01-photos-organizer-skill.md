# Photos Organizer Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Write a `photos-organizer` Claude Code skill that guides Claude through organizing a Google Photos Takeout export into descriptively named trip folders.

**Architecture:** A single `SKILL.md` file installed at `.claude/skills/photos-organizer/SKILL.md`. The skill documents 7 sequential phases with exact commands and code snippets. No new Python code — the skill drives existing `photos.py` subcommands.

**Tech Stack:** Claude Code skills system, existing `photos.py` CLI, SQLite (`output/photos.db`), Python 3.14, Qwen2.5-VL (via venv)
...
|------|--------|----------------|
| `.claude/skills/photos-organizer/SKILL.md` | Create | Full 7-phase skill document |
| `.claude/plugin/skills/photos-organizer/SKILL.md` | Create | Plugin copy (mirrors above — required for skill discovery) |
...
**Files:**
- Create: `.claude/skills/photos-organizer/SKILL.md`
- Create: `.claude/plugin/skills/photos-organizer/SKILL.md`

- [ ] **Step 1: Create the skill directory and write SKILL.md**

Write `.claude/skills/photos-organizer/SKILL.md`:
...
```bash
mkdir -p .claude/plugin/skills/photos-organizer
cp .claude/skills/photos-organizer/SKILL.md .claude/plugin/skills/photos-organizer/SKILL.md
```

- [ ] **Step 3: Verify skill is discoverable**

```bash
ls .claude/skills/photos-organizer/SKILL.md
ls .claude/plugin/skills/photos-organizer/SKILL.md
```
...
```bash
git add .claude/skills/photos-organizer/SKILL.md .claude/plugin/skills/photos-organizer/SKILL.md
git commit -m "feat: add photos-organizer skill"
```
